...
class ConfigManager():
    ...
    def setup_secure_directory_interactive(self):
        ...
        secure_directory = input("Chemin du répertoire sécurisé (appuyez sur Entrée pour utiliser le répertoire par défaut) : ").strip()
    ...
    def setup_secure_directory(self):
        ...
        try:
            ...
            os.chmod(self.secure_directory, 0o700)  # Permissions : lecture/écriture/exécution uniquement pour le propriétaire
        except Exception as e:
            logging.error(f"Erreur lors de la configuration du répertoire sécurisé : {e}")
            raise
    ...
    def secure_file(self, filepath):
        ...
        try:
            if os.path.exists(filepath):
                os.chmod(filepath, 0o600)  # Lecture/écriture uniquement pour le propriétaire
                print(f"Permissions restrictives appliquées pour le fichier : {filepath}")
            else:
