...
class MainMenu():
    ...
    def main_menu(self):
        ...
        while True:
            ...
            print(f"Utilisateur actif : {self.auth.user.current_user['username']} (rôle: {self.user.current_user['role']})")
            options = [
            ...
            if choice == 1:
                ...
                pwd = self.storage.reveal_password("passwords.txt", idx, self.enc, self.auth)
                ...
                result = self.report.remind_password_update(self.password_history) if hasattr(self.report, "remind_password_update") else None
                if result:
                ...
                records = self.storage.load_passwords(os.path.join(self.storage.secure_directory, "passwords.txt"))
                ...
                result = self.report.auto_breach_check(self.storage, self.enc, self.auth)
