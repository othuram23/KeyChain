...
class PasswordGenerator():
    ...
    def get_strength_message(entropy: float) -> str:
        ...
        if entropy < 28:
            ...
            return Fore.CYAN + "Moyen : Ce mot de passe est raisonnable mais améliorable."
        elif entropy < 80:
            return Fore.GREEN + "Fort : Ce mot de passe est robuste."
        else:
            return Fore.GREEN + Style.BRIGHT + "Très fort : Ce mot de passe est extrêmement robuste."
    ...
    def generate_password(length: int = 12, use_upper: bool = True, use_lower: bool = True,
        ...
        ambiguous_chars = PasswordGenerator.get_ambiguous_chars() if exclude_ambiguous else set()
        
        upper_chars = ''.join(c for c in string.ascii_uppercase if c not in ambiguous_chars) if use_upper else ""
        lower_chars = ''.join(c for c in string.ascii_lowercase if c not in ambiguous_chars) if use_lower else ""
        digit_chars = ''.join(c for c in string.digits if c not in ambiguous_chars) if use_digits else ""
        special_chars = ''.join(c for c in string.punctuation if c not in ambiguous_chars) if use_special else ""

        required_types = sum([bool(upper_chars), bool(lower_chars), bool(digit_chars), bool(special_chars)])
        
        if length < required_types:
            raise ValueError(f"La longueur doit être au moins {required_types} pour inclure chaque type.")

        password_chars = []
        if upper_chars: password_chars.append(secrets.choice(upper_chars))
        if lower_chars: password_chars.append(secrets.choice(lower_chars))
        if digit_chars: password_chars.append(secrets.choice(digit_chars))
        if special_chars: password_chars.append(secrets.choice(special_chars))
    ...
    def generate_password_menu(self):
        begin_with_personal_values: bool = input(
        ...
        if begin_with_personal_values:
            ...
            pool = ""
            if use_upper:
                pool += ''.join(c for c in string.ascii_uppercase if c not in ambiguous_chars)
            if use_lower:
                pool += ''.join(c for c in string.ascii_lowercase if c not in ambiguous_chars)
            if use_digits:
                pool += ''.join(c for c in string.digits if c not in ambiguous_chars)
            if use_special:
                pool += ''.join(c for c in string.punctuation if c not in ambiguous_chars)
        else:
        ...
        self.storage.add_password_to_history(password)  # Ajout du mot de passe à l'historique

        directory: str = self.conf.change_directory()  # Correction de l'appel de change_directory
        if directory != 'C:/Users/Admin/':
            ...
            self.storage.save_password_to_file(password=password, filename='passwords.txt')
            print("\nMot de passe sauvegardé avec succès!")
    ...
    def generate_password_from_pattern(pattern, exclude_ambiguous=False):
        ...
        supported = {'A', 'a', '9', '@'}
        for char in pattern:
            if char not in supported and char not in string.punctuation and not char.isalnum():
                raise ValueError(f"Caractère non supporté dans le motif : {char}")

        # 2. Exclusion des caractères ambigus
        ambiguous = PasswordGenerator.get_ambiguous_chars() if exclude_ambiguous else set()
        char_map = {
