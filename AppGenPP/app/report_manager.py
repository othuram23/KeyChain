...
class ReportManager():
    ...
    def analyze_saved_passwords(self, storage: StorageManager, enc: EncryptionManager, auth: AuthManager):
        ...
        if os.path.exists(filepath):
            ...
            for i, r in enumerate(records, 1):
                ...
                status = "Compromis" if pwd != "Inaccessible" and self.eval.check_password_breach(pwd) else "Sûr"
                report.append({"index": i, "password": '*' * len(pwd), "strength": strength, "status": status})
                print(f"{i}. {'*' * len(pwd)} - Force : {strength} - Statut : {status}")
            report_path = os.path.join(storage.secure_directory, "password_analysis_report.json")
    ...
    def generate_detailed_report(self, storage: StorageManager, enc: EncryptionManager, auth: AuthManager):
        filepath = os.path.join(storage.secure_directory, "passwords.txt")
    ...
    def search_password(self, keyword: str):
        ...
        if os.path.exists(filepath):
            results = [r for r in self.storage.load_passwords(filepath) if keyword.lower() in json.dumps(r).lower()]
          
            if results:
           
                for i, r in enumerate(results, 1):
                    print(f"{i} - Index: {r.get('index')}, Tags: {r.get('tags')}, Mot de passe masqué: {self.conf.mask_password(r.get('encrypted_password'))}")
