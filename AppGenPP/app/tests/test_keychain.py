...
class TestPasswordGenerator(unittest.TestCase):
    ...
    def test_generate_password(self):
        password = self.generator.generate_password(length=12, use_upper=True, use_lower=True, use_digits=True, use_special=True)
...
class TestEncryptionManager(unittest.TestCase):
    ...
    def test_hash_password(self):
        ...
        self.assertTrue(hashed.startswith("$2b$") or hashed.startswith("$argon2") or hashed.startswith("$pbkdf2"))

    def test_derive_key_from_master_password(self):
        master_password = "masterpassword"
        salt = b"random_salt"
        key = self.enc.derive_key_from_master_password(master_password, salt, key_length=16)
...
class TestPasswordCriteria(unittest.TestCase):
    ...
    def test_validate_password_criteria(self):
        ...
        self.assertTrue(self.generator.validate_password_criteria(12, True, True, True, True))
        self.assertFalse(self.generator.validate_password_criteria(0, True, True, True, True))
        self.assertFalse(self.generator.validate_password_criteria(12, False, False, False, False))
...
class TestMasterPasswordManager(unittest.TestCase):
    def setUp(self):
        ...
        self.manager._master_hash = self.manager.pwd_context.hash("masterpassword") # simule un hash déjà présent
